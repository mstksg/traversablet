...
instance (Traversable t, MonadError e t, Monad m) => MonadError e (TraversableT t m) where
    throwError = TraversableT . pure . throwError
    catchError x f = TraversableT $ do
        x' <- runTraversableT x
        let y = sequence $ fmap pure x' `catchError` \e -> pure (f e)
        join <$> runTraversableT y

instance (Alternative t, Traversable t, Monad t, Monad m) => Alternative (TraversableT t m) where
    empty = TraversableT $ pure empty
    x <|> y = TraversableT $ do
        x' <- runTraversableT x
        let c = (pure . pure <$> x') <|> pure (runTraversableT y)
        join <$> sequence c

instance (MonadPlus t, Traversable t, Monad t, Monad m) => MonadPlus (TraversableT t m) where
    mzero = empty
    mplus = (<|>)
